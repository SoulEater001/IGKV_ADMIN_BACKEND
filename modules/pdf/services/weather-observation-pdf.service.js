...
import { displayValue, escapeHtml, formatDate, toNumber } from "../utils/weather-pdf-format.utils.js";
export class WeatherObservationPdfService {
    ...
    // Observation report
    async getDailyWeatherReport(stationId, issueDate, fromDate, toDate) {
        const station = await this.repository.getStation(stationId);
        if (!station) {
            throw new Error('Weather station not found');
        }
        const rows = await this.repository.getObservations(stationId, issueDate, fromDate, toDate);
    ...
    async generateDailyWeatherPdf(stationId, reportType, weatherDataType, issueDate, fromDate, toDate) {
        const report = await this.getDailyWeatherReport(stationId, issueDate, fromDate, toDate);
        const html = await this.buildHtml(report, reportType, weatherDataType);
        return this.renderer.render(html);
    }
    async generateDailyWeatherHtml(stationId, issueDate, fromDate, toDate) {
        const report = await this.getDailyWeatherReport(stationId, issueDate, fromDate, toDate);
        return this.buildHtml(report, 'WEATHER_DATA', 'OBSERVATION');
    }
    async buildHtml(report, reportType, weatherDataType) {
