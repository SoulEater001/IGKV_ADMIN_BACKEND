...
import type {
  ...
  WeatherAdvisoryReport,
  WeatherPdfStationContext
} from '../types/weather-pdf.types.ts';

import {
  calculateWeatherMean,
  calculateWeatherTotal,
  roundValue
} from '../utils/weather-pdf.utils.ts';
import { displayValue, escapeHtml, toNumber, formatDate, formatRange } from '../utils/weather-pdf-format.utils.ts';
import { getMinMax } from '../utils/weather-pdf-format.utils.ts';
...
export class WeatherPdfService {
  ...
  // Forecast report
  async getDailyWeatherForecastReport(
    stationId: number,
    issueDate: string,
    stateLgCode: number,
    fromDate?: string,
  ...
  ): Promise<DailyWeatherForecastReport> {
    ...
    const location =
      await this.repository.getLocationNames(
        stateLgCode,
        districtLgCode ?? null,
        blockLgCode ?? null
      );
    ...
    const rows =
      await this.repository.getForecasts(
        stationId,
        issueDate,
        stateLgCode,
        fromDate,
  ...
  async generateDailyWeatherForecastHtml(
    stationId: number,
    issueDate: string,
    stateLgCode: number,
    fromDate?: string,
    ...
    const report =
      await this.getDailyWeatherForecastReport(
        stationId,
        issueDate,
        stateLgCode,
        fromDate,
  ...
  private async buildForecastHtml(report: DailyWeatherForecastReport, reportType: string, weatherDataType: string): Promise<string> {
  ...
  `)
    ...
    const blockHeading =
      report.blockLgCode && report.blockNameEn
